...
| API Key 管理 | 控制台创建 / 撤销；落盘只存哈希，明文仅在创建时返回一次 |
...
> `manage.py` 按脚本所在目录定位二进制与数据目录，克隆后无需改路径。默认监听 `0.0.0.0:4141`。
...
| 变量 | 是否需要 | 值 / 说明 |
|------|----------|-----------|
| `M365_ADMIN_PASSWORD` | 建议 | 设置一个至少 6 位的密码；不设则使用默认 `admin123`，首次登录会要求改密。 |
...
| `M365_AUTO_CLEANUP_KEEP_N` | `5` | 最多保留的云端对话数 |
...
1. **显式会话（`X-M365-Session-Id`）**：请求头显式指定的会话 ID 优先级最高，由调用方决定续接哪条云端对话。
2. **内容键前缀命中**：当请求的消息序列与某条已记录会话的历史**完全一致**，且 IP/UA 指纹一致时，直接复用该会话。此时返回的 `HistoryLen` 表示「云端对话已包含的消息条数」，上层据此只发送 `messages[HistoryLen:]` 增量。
3. **相似度兜底**：若消息不是严格前缀，但与某条最近活跃会话的 Jaccard 相似度超过阈值（默认 0.6），仍复用该会话。
4. **兜底新建**：都未命中时按 `user` / IP+UA 或轮询新建会话。

几个特性由此而来：

- **同 IP/UA 复用**：内容前缀只在同一客户端指纹下命中，避免短消息互串。
...
│   │   ├── openai_chat.go        # OpenAI chat 引擎（从 server.go 拆出）
│   │   ├── session_resolver.go   # 内容键会话复用（API Key + IP/UA）
│   │   ├── auto_cleanup.go        # 云端对话自动清理
│   │   ├── usage.go               # usage.jsonl 用量统计
│   │   └── ...                    # 工具调用、协议转换、代理池、密钥管理等
│   ├── chathub/           # M365 Copilot ChatHub WebSocket 客户端
│   ├── auth/              # OAuth / PKCE
│   ├── mcp/               # 实验性 MCP 工具网关（当前未挂到 HTTP 路由）
│   └── outbound/          # HTTP 代理池
...
- **默认监听**：直接运行二进制默认 `127.0.0.1:4141`；`manage.py` 默认 `0.0.0.0:4141`。
- **首次登录强制改密**：使用默认密码完成首次登录后必须修改管理员密码。
- **API Key**：落盘只存哈希，明文仅在创建时返回一次。
- **数据文件**：账号 Token、会话、API Key 等以 `0600` 写入。请定期备份数据目录。
...
网关默认按内容（上下文前缀 / 相似度）自动复用会话。当你希望在客户端侧显式控制会话与云端对话的对应关系时，携带 `X-M365-Session-Id` 请求头即可。
